...
- The standard tidyverse line is kings <- read_csv(). Thus, underscore is a tidyverse function, which means in this case that kings2 and kings4 are tidyverse functions, whereas the functions using the dot instead of the underscore is part of the normal R packages. Also, the only difference between kings2 and kings4 is the number two in csv2, which differs depending on whether it is a 'danish' file (data divided by semicolons) or an 'american/Canadian' file (data divided by commas). In my case my data is divided by semicolons, so I will be using kings4 <- read_csv2("data/Danish_Monarchs.csv"). 

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
- We create a tibble on the 'kings' that also behaves as a data frame, which contains 54 rows and 6 columns
...
- You can use glimpse(), summarize() or view()
...
kings%>%
  ...
  summarise(average_rule = mean(duration, na.rm = TRUE))
...
kings_duration %>%
  mutate(mid_year= Reign_start + (Reign_end - Reign_start) /2) %>% 
