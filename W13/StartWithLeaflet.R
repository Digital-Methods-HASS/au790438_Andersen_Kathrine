...
leaflet() %>%
  ...
  setView(lng = 151.005006, lat = -33.9767231, zoom = 12)              # set the location of the map 
# Question 1: What is the order of longitude and latitude in the setView() function?
# Longitude first, which goes from 0-180 degrees, and second the latitude which goes from 0-90 degrees.
...
leaflet() %>% 
  addTiles() %>% 
  setView( lng = 2.34, lat = 48.85, zoom = 12 ) %>%  # let's use setView to navigate to our area
...
# Clustering is very useful in interactive maps because of several things. 
# First of all it reduces visual clutter, making the map cleaner and easier to interpret. 
# With hundreds or maybe thousands of markers, the map quickly becomes unreadable.
# Second, it alsp makes it easier to notice spatial patterns.
# Third, it scales well with large data sets. 
...
saveWidget(DANmap_cluster, "DANmap_cluster.html", selfcontained = T)
